...
class pyNVTBase:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.threshold              = kwargs["threshold"] if "threshold" in kwargs else 2
        # Atlas
    ...
    def load_overlay(self):
        ...
        if self.data_type == 'UNDER_ONLY':
            ...
            if not os.path.isfile(self.path_overlay):
                raise Exception('[[[ERROR]]] [load_overlay] File Does not Exist [file path={}]'.format(self.path_overlay))
    ...
    def load_afni_overlay(self):
        ...
        # print "[load_afni_overlay] Brick_label=\n{}".format(afni.brick_label)
    ...
    def set_selected_slices(self):
        ...
        if start_slice <= 0 and end_slice <= 0:
            ...
            if start_slice >= end_slice:
                ...
                selected_slices = None
                raise Exception("[[[ERROR]]] [set_selected_slices] Slice Range should be less than {}.".format(max_slice))
            else:
                ...
                if num_slices == 0:
                    selected_slices = None
                    raise Exception("[[[ERROR]]]  [set_selected_slices] Space should be less than number of slices (space:{}).".format(self.space))
                else:
    ...
    def processing_data_intensity(self):
        ...
        if self.intensity_max == 0 and self.intensity_min == 0:
            ...
            # Return image after stretching or shrinking its intensity levels.
            self.data_underlay = exposure.rescale_intensity(self.data_underlay, in_range=(val_intensity_min, val_intensity_max))

    # -------------------------------------------------------
    #  Set Color Map for AFNI
    # -------------------------------------------------------
    ...
    def set_overlay_range(self):
        ...
        if self.value_range == 0:
            ...
            if False in check_overlay:
                max = abs(np.nanmax(self.data_overlay))
                min = abs(np.nanmin(self.data_overlay))
            else:
                max = 0
                min = 0
    ...
    def save_figure(self, figure, file_nm):
        try:
            ...
            print ("\n** [save_figure] {}".format(path_figure))
    ...
    def make_label_legend(self):
        ...
        for idx in label.keys():
            ...
            ax_leg.text(text_pos_x, y, label[idx][0], fontsize=font_size, horizontalalignment='left', verticalalignment='center')
    ...
    # -------------------------------------------------------
    def make_bar_legend(self, number_columns=2, cell_width=250, swatch_width=50, cmap=''):

        # Find the min and max of all colors for use in setting the color scale.
        v_min = -self.value_range
        v_max = self.value_range
        ...
        cb = colorbar.ColorbarBase(ax_leg, cmap=cmap, norm=norm, orientation='horizontal')
        plt.rcParams['savefig.facecolor'] = "white"
...
class pyNVT(pyNVTBase):
    ...
    def load_data(self):
        ...
        # sform
        self.sform_code = header['sform_code']
        print("[load_data] sform_code == [{}]".format(self.sform_code))
    ...
    def make_figure(self):
        ...
        norm = None
        if self.data_type == 'AFNI' or self.data_type == 'ICA':
            plt.register_cmap(name='shihlab', data=self.get_color_map())
        elif self.data_type == 'ATLAS':
            color_map, norm = self.get_color_map_from_label()
                ...
                try:
                    ...
                    if self.data_type != 'UNDER_ONLY':
                        ...
                        if self.data_type == 'AFNI' or self.data_type == 'ICA':
                            ...
                            if self.choose_axis == 'x':
                                ax.imshow(self.data_overlay[i, :, :].T, cmap='shihlab', vmax=self.max_value, vmin=self.min_value)
                            elif self.choose_axis == 'y':
                                ax.imshow(self.data_overlay[:, i, :].T, cmap='shihlab', vmax=self.max_value, vmin=self.min_value)
                            elif self.choose_axis == 'z':
                                ax.imshow(self.data_overlay[:, :, i].T, cmap='shihlab', vmax=self.max_value, vmin=self.min_value)
                            ...
                            if self.choose_axis == 'x':
                                ax.imshow(self.data_overlay[i, :, :].T, interpolation='nearest', norm=norm,
                                          cmap=color_map, alpha=self.transparent)
                            elif self.choose_axis == 'y':
                                ax.imshow(self.data_overlay[:, i, :].T, interpolation='nearest', norm=norm,
                                          cmap=color_map, alpha=self.transparent)
                            elif self.choose_axis == 'z':
